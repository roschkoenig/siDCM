function [E,V] = spm_si_cmc_priors(A,B,C,structure)
...
% scale structure net by max
%-------------------------------------------------------------------------- 
structure.net = (structure.net+structure.net')/2;
structure.net = structure.net/(max(structure.net(:)));
...
Q     = sparse(n,n);
for i = 1:length(A)
      A{i} = ~~(A{i}.*structure.net);
    E.A{i} = A{i}*32 - 32;
    V.A{i} = A{i}.*double(structure.smax)./(1+exp(double(structure.alpha)-double(structure.delta)*structure.net))/16;
    Q      = Q | A{i};
end
